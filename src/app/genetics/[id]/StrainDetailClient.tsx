...
import { Video } from "@/lib/archive";

export default function StrainDetailClient({ strain, relatedVideo }: { strain: Strain, relatedVideo: Video | undefined }) {
    ...
    <main className="min-h-screen bg-background relative overflow-hidden pb-24">
      ...
      <div className="px-4 md:px-8 max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        >
          ...
          <div className="mb-10">
            <h2 className="font-mono text-xs uppercase tracking-widest text-lab-text mb-4 flex items-center gap-2">
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-10">
            <div className="bg-white border border-lab-border p-4 rounded-lg shadow-sm">
              <dt className="font-mono text-[10px] uppercase text-lab-text mb-1 flex items-center gap-1"><Sprout size={12}/> Type</dt>
              <dd className="font-bold text-sm m-0">{strain.type}</dd>
            </div>
            <div className="bg-white border border-lab-border p-4 rounded-lg shadow-sm border-l-4 border-l-primary">
              <dt className="font-mono text-[10px] uppercase text-lab-text mb-1 flex items-center gap-1"><Activity size={12}/> THC Range</dt>
              <dd className="font-bold text-sm text-primary m-0">{strain.thc}</dd>
            </div>
            <div className="bg-white border border-lab-border p-4 rounded-lg shadow-sm">
              <dt className="font-mono text-[10px] uppercase text-lab-text mb-1 flex items-center gap-1"><Droplet size={12}/> Yield</dt>
              <dd className="font-bold text-sm m-0">{strain.yield}</dd>
            </div>
            <div className="bg-white border border-lab-border p-4 rounded-lg shadow-sm">
              <dt className="font-mono text-[10px] uppercase text-lab-text mb-1 flex items-center gap-1"><Beaker size={12}/> Flowering</dt>
              <dd className="font-bold text-sm m-0">{strain.flow}</dd>
            </div>
            {strain.stretch && (
              <div className="bg-white border border-lab-border p-4 rounded-lg shadow-sm">
                <dt className="font-mono text-[10px] uppercase text-lab-text mb-1 flex items-center gap-1"><Ruler size={12}/> Stretch</dt>
                <dd className="font-bold text-sm m-0">{strain.stretch}</dd>
              </div>
            )}
          </dl>
          ...
          <div className="bg-foreground text-background rounded-xl p-6 shadow-xl mb-10 relative overflow-hidden">
            ...
            <h2 className="font-mono text-xs uppercase tracking-widest text-lab-bg mb-4 flex items-center gap-2">
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
              ...
              <div>
                ...
                <dd className="font-bold text-accent m-0">{strain.aroma}</dd>
              </div>
            </dl>
          </div>
